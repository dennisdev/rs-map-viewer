export class Tile {
    ...
    constructor(hash: number | Tile, y?: number, z?: number) {
        if (typeof hash === 'number' && typeof y === 'number' && typeof z === 'number') {
            // Constructor with x, y, and z
            this.hash = y | (hash << 14) | (z << 28);
        } else if (typeof hash === 'number' && typeof y === 'number') {
            // Constructor with x and y (default z = 0)
            this.hash = y | (hash << 14);
        } else if (hash instanceof Tile) {
            // Constructor with another Location
            this.hash = hash.getWorldY() | (hash.getWorldX() << 14) | (hash.getPlane() << 28);
        } else if (typeof hash === 'number') {
            // Constructor with a hash
            this.hash = (hash >> 14) & 16383 | (hash & 16383) | ((hash >> 28) & 3) << 28;
        } else {
    ...
    transform(diffX: number, diffY: number, diffZ?: number): Tile {
        if (typeof diffZ === 'number') {
            return new Tile(this.getWorldX() + diffX, this.getWorldY() + diffY, this.getPlane() + diffZ);
        } else {
    ...
    getLocalX(l: Tile): number {
        return this.getWorldX() - 8 * (l.getChunkX() - 6);
    }

    getLocalY(l: Tile): number {
        return this.getWorldY() - 8 * (l.getChunkY() - 6);
    }
    ...
    getLocalTileId(tile: Tile): number {
        return ((this.getLocalX(tile) & 7) << 4) | (this.getLocalY(tile) & 7);
    }

    hashInRegion(): number {
        return ((this.getWorldX() & 63) << 6) | (this.getWorldY() & 63) | ((this.getPlane() & 3) << 12);
    }
    ...
    deltaAbsolute(b: Tile): Tile {
        return new Tile(Math.abs(b.getWorldX() - this.getWorldX()), Math.abs(b.getWorldY() - this.getWorldY()));
    }
    ...
    equals(arg1: number | Tile, y?: number, plane?: number): boolean {
        ...
        } else if (arg1 instanceof Tile && typeof y === 'undefined' && typeof plane === 'undefined') {
    ...
    isInside(other: Tile, thisSize: number): boolean {
        ...
        const insideHorizontal = other.getWorldX() >= this.getWorldX() && other.getWorldX() <= this.getWorldX() + (thisSize - 1);
        const insideVertical = other.getWorldY() >= this.getWorldY() && other.getWorldY() <= this.getWorldY() + (thisSize - 1);
