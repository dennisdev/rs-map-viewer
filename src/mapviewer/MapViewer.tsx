    ...
    if (this.movementAnimation !== -1) {
      ...
      if (!anim.isAnimMaya() && anim.frameIds) {
        ...
        if (
          ...
          this.movementFrameTick > anim.frameLengths[this.movementFrame]
        ) {
  ...
  render(
    ...
    if (this.pickX !== -1 && this.pickY !== -1) {
      ...
      readPixelsAsync(
        ...
        const closestInteractions: number[] = new Array(INTERACTION_SIZE).fill(
          -1
        );
    ...
    for (let i = this.visibleChunkCount - 1; i >= 0; i--) {
      ...
      chunk.npcs.forEach((npc, i) => {
        ...
        const anim =
          npc.data.walkAnim && npc.movementAnimation === npc.def.walkSequence
            ? npc.data.walkAnim
            : npc.data.idleAnim;
    ...
    for (let i = 0; i < this.visibleChunkCount; i++) {
      ...
      for (const animatedModel of chunk.animatedModels) {
        if (animatedModel.framesAlpha) {
          ...
          (drawCall as any).offsets[animatedModel.drawRangeAlphaIndex] =
            frame[0];
          (drawCall as any).numElements[animatedModel.drawRangeAlphaIndex] =
            frame[1];
...
function MapViewerApp() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (cx && cy && cz) {
        const pos: vec3 = [-parseFloat(cx), parseFloat(cy), -parseFloat(cz)];
