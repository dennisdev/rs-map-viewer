...
export class Compression {
    ...
    public static initWasm() {
        ...
        const gzipPromise = initWasmGzip(wasmGzipUrl).then(
            (output) => (Compression.wasmGzipLoaded = true)
        );
        const bzip2Promise = Bzip2.init().then(
            (instance) => (Compression.wasmBzip2 = instance)
        );
    ...
    public static decompressBzip2(
        ...
        if (Compression.wasmBzip2) {
            return new Int8Array(
                Compression.wasmBzip2.decompress(compressedBzip, actualSize, {
                    small: false,
                }).buffer
            );
        }
